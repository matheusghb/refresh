...
EOS "vencerá" por conseguir realmente destruir tudo, só sobrando os jogadores com a ferramenta para recriar a realidade. -> derrotar EOS em um combate físico agora que todos estão sincronizados com ele (Todos estão conectados ao presente de EOS) -> Acessar o presente de EOS por meio de um dos jogadores, que ficará preso na quarta dimensão -> encontrar o narrador, a forma ativa inversa do EOS e por meio dele alcançar a quarta dimensão por um momento temporário. Ao interagir com a quarta dimensão, o narrador se torna exposto para EOS e vai morrer para ele. A partir daí, é um ponto sem retorno. -> EOS possui uma "carametade" originada do mesmo ato que o selou, sua versão "presente" selada no backdoor que passou a ver a história por fora e se mantendo como uma versão invertida de si. -> para encontrar o backdoor, terão que falar diretamente com o criador: o rogue na cabeça da seer, escondido em derse, agora espalhado pelo furtherest ring. A bellax vai morrer para separar seu corpo e o do rogue e disso poderá guiar os jogadores ate ele, sendo o caso até se ela der godtier. ->  